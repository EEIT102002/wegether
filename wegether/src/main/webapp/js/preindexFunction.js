function indexSelectFromDB(jsonArray){
	...
	D = date.getDate() + ' ';
	str1 +="<div class='col-xs-12 col-sm-6 col-md-4 item'><figure style='display:flex;margin:5px 0;' ><img alt='' class='img-circle img-responsive' style='height: 50px;width:50px;' src=\""+jsonArray[i][3]+"\"/><figcaption style='line-height:50px;flex:0.8;padding-left:20px;'>"+jsonArray[i][2]+"<input type='hidden' value='"+jsonArray[i][4]+"' class='hidememid'/></figcaption></figure><figure><a href='activityPage.controller?actid="+jsonArray[i][0].id+"'>";
	str1 += "<img alt='' class='img-responsive' style='border-radius:5px;' src=\""+jsonArray[i][1]+"\"/></a>";
	str1+="<figcaption style='margin-top:5px;'>";
	str1+="活動名稱:"+jsonArray[i][0].title+"<br>"+"活動地點:"+array_for_city[jsonArray[i][0].city-1]+"<br>"+"活動時間:"+(Y+M+D)+"<br>"+"活動內容:"+jsonArray[i][0].content;
