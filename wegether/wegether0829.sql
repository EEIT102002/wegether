...
begin
	...
	begin
		...
		select memberid, memberidf, i.state 
		from inserted i
	end
end
go
...
begin 
	...
	if(@f_state = 0)
	begin
		set @content = '好友邀請 '+ (select nickname from  member where id = @f_memberid)
		insert into notice (memberid, content, friendid) values (@f_friendid, @content, @f_id)
	end
end
go
...
begin
	...
	begin 
		...
		set @content = (select nickname from  member where id = (select memberidf from inserted))+
			'同意好友請求';

		insert into notice (memberid, content, friendid)
		select memberid , @content , id
		from inserted
...
as
begin
	insert into notice (memberid, inviteid, content )
	select invitedid , id, 
	(select nickname from  member where id = i.memberid)+'推薦你參加'+
	(select title from activity where id = i.activityid)
	from inserted i
end
go
...
begin
	...
	insert into notice(memberid, attendid , content, noticetime)
	select @hostid , id ,
		(select nickname from member where id = i.memberid )+'申請參加'+@title
		,createtime
	from inserted i
end
...
begin 
	...
	insert into notice (memberid, attendid, content, noticetime)
	select memberid, id, 
		(select nickname from  member where id = @hostid)+'邀請你參加'+ @title
		,createtime
	from inserted
end
end
...
select u.id, memberid, dstate, istate ,u.activityid, act.hostid , act.title
into #uattend
from (
...
insert into  notice  (memberid , attendid , content)
select  memberid, id , title+
	(case istate when 1 then '報名成功' when 2 then '報名失敗' end)
from #uattend
where dstate = 0
...
select  hostid, id , 
	...
	(case istate when 1 then '接受邀請' when 2 then '拒絕邀請' end)
from #uattend
where dstate = 3
end
go
...
begin
	...
	select @actorname = nickname+'有新的文章'
	from member 
	where id = (select memberid from inserted)

	insert into notice (memberid, articleid , content, noticetime)
	select memberidf, i.id, @actorname, i.createtime
	from friend
	join inserted i
	...
	insert into notice (memberid, articleid , content, noticetime)
	select fanid, i.id, @actorname, i.createtime
	from trackmember t
	join inserted i 
...
begin
	...
	select @hostname = nickname+'發起新活動'
	from member 
	where id = (select hostid from inserted)

	insert into notice(memberid,content,noticetime , activityid)
	select memberidf, @hostname, i.createtime ,i.id
	from friend f
	join inserted i
	...
	insert into notice (memberid, content , noticetime, activityid)
	select fanid, @hostname, (select createtime from inserted),
...
begin
	...
	begin
		...
		insert into notice (memberid, content , noticetime)
		select memberid, @title+' 活動取消', @date
		from attend
	...
	begin
		...
		on notice.memberid = a.memberid and notice.activityid = a.activityid
		when matched then
			update
			set content = @title +'活動變更',
				noticetime = @date,
				state = 0
		when not matched then
			insert (memberid, activityid, content)
			values(a.memberid, a.activityid, @title +'活動變更');
	end
end
end
go
	...
	begin
		...
		when matched then 
			...
			set n.noticetime = @date, 
				...
				n.state = 0
		when not matched then
			insert (memberid, activityid, content , noticetime, msgcount, msgcountu)
			values (a.hostid, a.id, a.title+'有新的留言', @date, 1, 1);
	end
	...
	begin
		...
		when matched then 
			...
			set n.noticetime = @date,
				...
				n.state = 0
		when not matched then 
			insert (memberid, articleid, noticetime, msgcount, msgcountu,content)
			values (a.memberid, a.id, @date,1,1, (select title from activity where id = a.activityid)+'的文章有新的留言');
	end
end
go
